...
int CalculateMaxWorkshops(Available_Workshops* ptr) {
    ...
    //change start timings to 24 Hr format
    for(int i=0; i<number_of_workshop; i++) {
        current_startTime = ptr->workshop_ptr[i].start_time;
        
        if(i+1<number_of_workshop) {
            //watching the next start time
            nextStartTime = ptr->workshop_ptr[i].start_time;
            
        }
    }

    cout << "STart time in 24 Hr:" << endl;
    for(int i=0; i<number_of_workshop; i++) {
        cout << ptr->workshop_ptr[i].start_time << " ";
    } cout << endl;
